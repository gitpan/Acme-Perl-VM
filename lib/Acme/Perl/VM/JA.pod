...
opcodeの持つ手続きは対応するppcodeであり，C<op_ppaddr>メンバで参照します。
...
ppcodeはopcodeオブジェクトのC<op_ppaddr>メンバを通じて取得・変更することができます。このC<op_ppaddr>をC<PL_check>というコンパイラフックテーブルを通じて変更し，プログラムの挙動を変える手法がB<PL_check hack>として知られています。たとえば，C<autobox>はこのC<PL_check>ハックを用いてプリミティブ値に対するメソッド呼び出しを実現しています。
